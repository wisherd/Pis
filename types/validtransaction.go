...
func (t Transaction) followsMinimumValues() error {
	...
	for _, sfo := range t.PisfundOutputs {
		// PisfundOutputs are special in that they have a reserved field, the
...
func (t Transaction) noRepeats() error {
	...
	siacoinInputs := make(map[PiscoinOutputID]struct{})
	...
	siafundInputs := make(map[PisfundOutputID]struct{})
