...
type (
	...
	FileContract struct {
		...
		ValidProofOutputs  []PiscoinOutput `json:"validproofoutputs"`
		MissedProofOutputs []PiscoinOutput `json:"missedproofoutputs"`
	...
	FileContractRevision struct {
		...
		NewValidProofOutputs  []PiscoinOutput `json:"newvalidproofoutputs"`
		NewMissedProofOutputs []PiscoinOutput `json:"newmissedproofoutputs"`
	...
	//
	// A transaction with a StorageProof cannot have any PiscoinOutputs,
	// PisfundOutputs, or FileContracts. This is because a mundane reorg can
...
func (fcid FileContractID) StorageProofOutputID(proofStatus ProofStatus, i uint64) PiscoinOutputID {
...
func Tax(height BlockHeight, payout Currency) Currency {
	...
	if height < TaxHardforkHeight {
		return payout.MulFloat(0.039).RoundDown(PisfundCount)
	}
	return payout.MulTax().RoundDown(PisfundCount)
}
