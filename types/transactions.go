...
var (
	...
	SpecifierPiscoinInput         = Specifier{'s', 'i', 'a', 'c', 'o', 'i', 'n', ' ', 'i', 'n', 'p', 'u', 't'}
	SpecifierPiscoinOutput        = Specifier{'s', 'i', 'a', 'c', 'o', 'i', 'n', ' ', 'o', 'u', 't', 'p', 'u', 't'}
	SpecifierPisfundInput         = Specifier{'s', 'i', 'a', 'f', 'u', 'n', 'd', ' ', 'i', 'n', 'p', 'u', 't'}
	SpecifierPisfundOutput        = Specifier{'s', 'i', 'a', 'f', 'u', 'n', 'd', ' ', 'o', 'u', 't', 'p', 'u', 't'}
...
type (
	...
	// PiscoinOutputID uniquely identifies a siacoin output
	PiscoinOutputID crypto.Hash
	// PisfundOutputID uniquely identifies a siafund output
	PisfundOutputID crypto.Hash
	...
	// self-dependent.
	Transaction struct {
		PiscoinInputs         []PiscoinInput         `json:"siacoininputs"`
		PiscoinOutputs        []PiscoinOutput        `json:"siacoinoutputs"`
		...
		PisfundInputs         []PisfundInput         `json:"siafundinputs"`
		PisfundOutputs        []PisfundOutput        `json:"siafundoutputs"`
	...
	// A PiscoinInput consumes a PiscoinOutput and adds the siacoins to the set of
	...
	// the output.
	PiscoinInput struct {
		ParentID         PiscoinOutputID  `json:"parentid"`
		UnlockConditions UnlockConditions `json:"unlockconditions"`
	}

	// A PiscoinOutput holds a volume of siacoins. Outputs must be spent
	...
	PiscoinOutput struct {
		Value      Currency   `json:"value"`
		UnlockHash UnlockHash `json:"unlockhash"`
	}

	// A PisfundInput consumes a PisfundOutput and adds the siafunds to the set of
	// siafunds that can be spent in the transaction. The ParentID points to the
	// output that is getting consumed, and the UnlockConditions contain the rules
	// for spending the output. The UnlockConditions must match the UnlockHash of
	// the output.
	PisfundInput struct {
		ParentID         PisfundOutputID  `json:"parentid"`
		UnlockConditions UnlockConditions `json:"unlockconditions"`
		ClaimUnlockHash  UnlockHash       `json:"claimunlockhash"`
	}

	// A PisfundOutput holds a volume of siafunds. Outputs must be spent
	// atomically; that is, they must all be spent in the same transaction. The
	// UnlockHash is the hash of a set of UnlockConditions that must be fulfilled
	// in order to spend the output.
	//
	// When the PisfundOutput is spent, a PiscoinOutput is created, where:
	//
	//     PiscoinOutput.Value := (PisfundPool - ClaimStart) / 10,000 * Value
	//     PiscoinOutput.UnlockHash := PisfundInput.ClaimUnlockHash
	//
	// When a PisfundOutput is put into a transaction, the ClaimStart must always
	// equal zero. While the transaction is being processed, the ClaimStart is set
	// to the value of the PisfundPool.
	PisfundOutput struct {
		Value      Currency   `json:"value"`
		UnlockHash UnlockHash `json:"unlockhash"`
...
// PiscoinOutputID returns the ID of a siacoin output at the given index,
// which is calculated by hashing the concatenation of the PiscoinOutput
// Specifier, all of the fields in the transaction (except the signatures),
// and output index.
func (t Transaction) PiscoinOutputID(i uint64) PiscoinOutputID {
	// Create the id.
	var id PiscoinOutputID
	h := crypto.NewHash()
	h.Write(SpecifierPiscoinOutput[:])
	t.marshalPisNoSignatures(h) // Encode non-signature fields into hash.
	...
	if build.DEBUG {
		verificationID := PiscoinOutputID(crypto.HashAll(
			SpecifierPiscoinOutput,
			t.PiscoinInputs,
			t.PiscoinOutputs,
			t.FileContracts,
			t.FileContractRevisions,
		...
		if id != verificationID {
			panic("PiscoinOutputID is not marshalling correctly")
		}
	}
...
// PisfundOutputID returns the ID of a PisfundOutput at the given index, which
// is calculated by hashing the concatenation of the PisfundOutput Specifier,
// all of the fields in the transaction (except the signatures), and output
// index.
func (t Transaction) PisfundOutputID(i uint64) PisfundOutputID {
	var id PisfundOutputID
	h := crypto.NewHash()
	h.Write(SpecifierPisfundOutput[:])
	t.marshalPisNoSignatures(h) // Encode non-signature fields into hash.
	encoding.WriteUint64(h, i)  // Writes index of this output.
	h.Sum(id[:0])
	...
	// same ids as the unoptimized code.
	if build.DEBUG {
		verificationID := PisfundOutputID(crypto.HashAll(
			SpecifierPisfundOutput,
			t.PiscoinInputs,
			t.PiscoinOutputs,
			t.FileContracts,
			t.FileContractRevisions,
		...
		if id != verificationID {
			panic("PisfundOutputID is not marshalling correctly")
		}
	}
	return id
}

// PiscoinOutputSum returns the sum of all the siacoin outputs in the
// transaction, which must match the sum of all the siacoin inputs. Piscoin
...
// created.
func (t Transaction) PiscoinOutputSum() (sum Currency) {
...
// PisClaimOutputID returns the ID of the PiscoinOutput that is created when
// the siafund output is spent. The ID is the hash the PisfundOutputID.
func (id PisfundOutputID) PisClaimOutputID() PiscoinOutputID {
	return PiscoinOutputID(crypto.HashObject(id))
}
