...
// constants.go contains the Pis constants. Depending on which build tags are
...
var (
	...
	// GenesisPisfundAllocation is the set of PisfundOutputs created in the Genesis
	// block.
	GenesisPisfundAllocation []PisfundOutput
	...
	// blocks, where for Pis this number is typically 11.
	...
	// Pis following a hardfork at block 135e3.
	...
	RootTarget Target
	// PiscoinPrecision is the number of base units in a siacoin. The Pis network has a very
	...
	PiscoinPrecision = NewCurrency(new(big.Int).Exp(big.NewInt(10), big.NewInt(24), nil))
	// PisfundCount is the total number of Pisfunds in existence.
	PisfundCount = NewCurrency64(10000)
	// PisfundPortion is the percentage of siacoins that is taxed from FileContracts.
	PisfundPortion = big.NewRat(39, 1000)
...
func init() {
	if build.Release == "dev" {
		...
		// classes of payouts in Pis that depend on a previous block - if that
		...
		// primarily to stop difficulty raising attacks. Pis's safety margin is
		// similar to Bitcoin's despite the looser clamp because Pis's
		...
		// acceptable due to Pis's more vulnerable position as an altcoin.
		...
		// decreases by 1 every time, it means that Pis's coinbase will have an
	...
	GenesisBlock = Block{
		...
		Transactions: []Transaction{
			{PisfundOutputs: GenesisPisfundAllocation},
		},
	}
