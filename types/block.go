...
// block.go defines the Block type for Pis, and provides some helper functions
...
type (
	...
	Block struct {
		...
		MinerPayouts []PiscoinOutput `json:"minerpayouts"`
...
// equation is:
//
//     coinbase := max(InitialCoinbase - height, MinimumCoinbase) * PiscoinPrecision
func CalculateCoinbase(height BlockHeight) Currency {
	...
	return NewCurrency64(base).Mul(PiscoinPrecision)
}

// CalculateNumPiscoins calculates the number of siacoins in circulation at a
// given height.
func CalculateNumPiscoins(height BlockHeight) Currency {
	deflationBlocks := BlockHeight(InitialCoinbase - MinimumCoinbase)
	avgDeflationPiscoins := CalculateCoinbase(0).Add(CalculateCoinbase(height)).Div(NewCurrency64(2))
	if height <= deflationBlocks {
		deflationPiscoins := avgDeflationPiscoins.Mul(NewCurrency64(uint64(height + 1)))
		return deflationPiscoins
	}
	deflationPiscoins := avgDeflationPiscoins.Mul(NewCurrency64(uint64(deflationBlocks + 1)))
	trailingPiscoins := NewCurrency64(uint64(height - deflationBlocks)).Mul(CalculateCoinbase(height))
	return deflationPiscoins.Add(trailingPiscoins)
}
...
// index.
func (b Block) MinerPayoutID(i uint64) PiscoinOutputID {
	return PiscoinOutputID(crypto.HashAll(
		b.ID(),
		i,
	))
}
