...
//    x
//
func TransactionGraph(sourceOutput PiscoinOutputID, edges []TransactionGraphEdge) ([]Transaction, error) {
	...
	txnInputs := make([][]PiscoinOutputID, biggest+1)
	txnInputs[0] = []PiscoinOutputID{sourceOutput}
	...
	for i < len(edges) {
		...
		for _, outputID := range txnInputs[j] {
			t.PiscoinInputs = append(t.PiscoinInputs, PiscoinInput{
				ParentID: outputID,
			})
		}
		...
		for i < len(edges) && edges[i].Source == current {
			t.PiscoinOutputs = append(t.PiscoinOutputs, PiscoinOutput{
				Value:      edges[i].Value,
		...
		for k := startingPoint; k < i; k++ {
			txnInputs[edges[k].Dest] = append(txnInputs[edges[k].Dest], t.PiscoinOutputID(uint64(k-startingPoint)))
		}
		ts[j] = t
		j++
	}
