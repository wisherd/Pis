...
type (
	...
	CoveredFields struct {
		...
		PiscoinInputs         []uint64 `json:"siacoininputs"`
		PiscoinOutputs        []uint64 `json:"siacoinoutputs"`
		...
		PisfundInputs         []uint64 `json:"siafundinputs"`
		PisfundOutputs        []uint64 `json:"siafundoutputs"`
	...
	// A PisPublicKey is a public key prefixed by a Specifier. The Specifier
	...
	// are three types that use UnlockConditions: PiscoinInputs, PisfundInputs,
	...
	// certain actions, such as spending a PiscoinOutput or terminating a
	// FileContract.
	//
	...
	UnlockConditions struct {
		Timelock           BlockHeight    `json:"timelock"`
		PublicKeys         []PisPublicKey `json:"publickeys"`
	...
	inputSignatures struct {
		...
		possibleKeys        []PisPublicKey
		usedKeys            map[uint64]struct{}
		index               int
	}
)

// Ed25519PublicKey returns pk as a PisPublicKey, denoting its algorithm as
// Ed25519.
func Ed25519PublicKey(pk crypto.PublicKey) PisPublicKey {
	return PisPublicKey{
...
func (t Transaction) SigHash(i int) (hash crypto.Hash) {
	...
	if cf.WholeTransaction {
		...
		for _, input := range cf.PiscoinInputs {
			t.PiscoinInputs[input].MarshalPis(h)
		}
		for _, output := range cf.PiscoinOutputs {
			t.PiscoinOutputs[output].MarshalPis(h)
		}
		for _, contract := range cf.FileContracts {
			t.FileContracts[contract].MarshalPis(h)
		}
		for _, revision := range cf.FileContractRevisions {
			t.FileContractRevisions[revision].MarshalPis(h)
		}
		for _, storageProof := range cf.StorageProofs {
			t.StorageProofs[storageProof].MarshalPis(h)
		}
		for _, siafundInput := range cf.PisfundInputs {
			t.PisfundInputs[siafundInput].MarshalPis(h)
		}
		for _, siafundOutput := range cf.PisfundOutputs {
			t.PisfundOutputs[siafundOutput].MarshalPis(h)
		}
		for _, minerFee := range cf.MinerFees {
			t.MinerFees[minerFee].MarshalPis(h)
		}
	...
	for _, sig := range cf.TransactionSignatures {
		t.TransactionSignatures[sig].MarshalPis(h)
	}
...
func (t Transaction) validCoveredFields() error {
	for _, sig := range t.TransactionSignatures {
		...
		fieldMaxs := []struct {
			...
			{cf.PiscoinInputs, len(t.PiscoinInputs)},
			{cf.PiscoinOutputs, len(t.PiscoinOutputs)},
			...
			{cf.PisfundInputs, len(t.PisfundInputs)},
			{cf.PisfundOutputs, len(t.PisfundOutputs)},
...
func (t *Transaction) validSignatures(currentHeight BlockHeight) error {
	...
	for i, input := range t.PiscoinInputs {
	...
	for i, input := range t.PisfundInputs {
		id := crypto.Hash(input.ParentID)
		_, exists := sigMap[id]
		if exists {
			return ErrDoubleSpend
