...
type (
	...
	BlockFacts struct {
		...
		PiscoinInputCount         uint64 `json:"siacoininputcount"`
		PiscoinOutputCount        uint64 `json:"siacoinoutputcount"`
		...
		PisfundInputCount         uint64 `json:"siafundinputcount"`
		PisfundOutputCount        uint64 `json:"siafundoutputcount"`
	...
	Explorer interface {
		...
		// PiscoinOutput will return the siacoin output associated with the
		// input id.
		PiscoinOutput(types.PiscoinOutputID) (types.PiscoinOutput, bool)

		// PiscoinOutputID returns all of the transaction ids associated with
		// the provided siacoin output id.
		PiscoinOutputID(types.PiscoinOutputID) []types.TransactionID
		...
		// PisfundOutput will return the siafund output associated with the
		// input id.
		PisfundOutput(types.PisfundOutputID) (types.PisfundOutput, bool)

		// PisfundOutputID returns all of the transaction ids associated with
		// the provided siafund output id.
		PisfundOutputID(types.PisfundOutputID) []types.TransactionID
