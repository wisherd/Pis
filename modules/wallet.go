...
type (
	...
	//
	// Fund type can either be 'PiscoinOutput', 'PisfundOutput', 'ClaimOutput',
	...
	// available. PiscoinInputs and PisfundInputs become available immediately.
	...
	// 'Sign', which will sign all inputs added via the 'FundPiscoins' or
	// 'FundPisfunds' call. All modifications are additive.
	//
	...
	TransactionBuilder interface {
		// FundPiscoins will add a siacoin input of exactly 'amount' to the
		...
		// transaction failed.
		FundPiscoins(amount types.Currency) error

		// FundPisfunds will add a siafund input of exactly 'amount' to the
		// transaction. A parent transaction may be needed to achieve an input
		...
		// failed.
		FundPisfunds(amount types.Currency) error
		...
		// AddPiscoinInput adds a siacoin input to the transaction, returning
		// the index of the siacoin input within the transaction. When 'Sign'
		// gets called, this input will be left unsigned.
		AddPiscoinInput(types.PiscoinInput) uint64

		// AddPiscoinOutput adds a siacoin output to the transaction, returning
		// the index of the siacoin output within the transaction.
		AddPiscoinOutput(types.PiscoinOutput) uint64
		...
		// AddPisfundInput adds a siafund input to the transaction, returning
		// the index of the siafund input within the transaction. When 'Sign'
		// is called, this input will be left unsigned.
		AddPisfundInput(types.PisfundInput) uint64

		// AddPisfundOutput adds a siafund output to the transaction, returning
		// the index of the siafund output within the transaction.
		AddPisfundOutput(types.PisfundOutput) uint64
		...
		// sign any of the inputs that were added by calling 'FundPiscoins' or
		// 'FundPisfunds'.
		AddTransactionSignature(types.TransactionSignature) uint64

		// Sign will sign any inputs added by 'FundPiscoins' or 'FundPisfunds'
	...
	KeyManager interface {
		...
		// LoadPisgKeys will take a set of filepaths that point to a siag key
		// and will have the siag keys loaded into the wallet so that they will
		// become spendable.
		LoadPisgKeys(crypto.TwofishKey, []string) error
	...
	Wallet interface {
		...
		// someone could result in 'outgoing: 12, incoming: 11'. Pisfunds are
		// not considered in the unconfirmed balance.
		UnconfirmedBalance() (outgoingPiscoins types.Currency, incomingPiscoins types.Currency, err error)
		...
		// SendPiscoins is a tool for sending siacoins from the wallet to an
		...
		SendPiscoins(amount types.Currency, dest types.UnlockHash) ([]types.Transaction, error)

		// SendPiscoinsMulti sends coins to multiple addresses.
		SendPiscoinsMulti(outputs []types.PiscoinOutput) ([]types.Transaction, error)

		// SendPisfunds is a tool for sending siafunds from the wallet to an
		// address. Sending money usually results in multiple transactions. The
		// transactions are automatically given to the transaction pool, and
		// are also returned to the caller.
		SendPisfunds(amount types.Currency, dest types.UnlockHash) ([]types.Transaction, error)
