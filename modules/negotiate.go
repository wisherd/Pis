...
const (
	...
	// NegotiateMaxPisPubkeySize defines the maximum size that a PisPubkey is
	// allowed to be when being sent over the wire during negotiation.
	NegotiateMaxPisPubkeySize = 1e3
...
// transaction.
func CreateAnnouncement(addr NetAddress, pk types.PisPublicKey, sk crypto.SecretKey) (signedAnnouncement []byte, err error) {
...
func DecodeAnnouncement(fullAnnouncement []byte) (na NetAddress, spk types.PisPublicKey, err error) {
