...
type (
	...
	ConsensusChange struct {
		...
		// PiscoinOutputDiffs contains the set of siacoin diffs that were applied
		// to the consensus set in the recent change. The direction for the set of
		// diffs is 'DiffApply'.
		PiscoinOutputDiffs []PiscoinOutputDiff
		...
		// PisfundOutputDiffs contains the set of siafund diffs that were applied
		// to the consensus set in the recent change. The direction for the set of
		// diffs is 'DiffApply'.
		PisfundOutputDiffs []PisfundOutputDiff

		// DelayedPiscoinOutputDiffs contains the set of delayed siacoin output
		// diffs that were applied to the consensus set in the recent change.
		DelayedPiscoinOutputDiffs []DelayedPiscoinOutputDiff

		// PisfundPoolDiffs are the siafund pool diffs that were applied to the
		// consensus set in the recent change.
		PisfundPoolDiffs []PisfundPoolDiff
	...
	// A PiscoinOutputDiff indicates the addition or removal of a PiscoinOutput in
	// the consensus set.
	PiscoinOutputDiff struct {
		Direction     DiffDirection
		ID            types.PiscoinOutputID
		PiscoinOutput types.PiscoinOutput
	}
	...
	// A PisfundOutputDiff indicates the addition or removal of a PisfundOutput in
	// the consensus set.
	PisfundOutputDiff struct {
		Direction     DiffDirection
		ID            types.PisfundOutputID
		PisfundOutput types.PisfundOutput
	}

	// A DelayedPiscoinOutputDiff indicates the introduction of a siacoin output
	// that cannot be spent until after maturing for 144 blocks. When the output
	// has matured, a PiscoinOutputDiff will be provided.
	DelayedPiscoinOutputDiff struct {
		Direction      DiffDirection
		ID             types.PiscoinOutputID
		PiscoinOutput  types.PiscoinOutput
	...
	// A PisfundPoolDiff contains the value of the siafundPool before the block
...
func (cc ConsensusChange) Append(cc2 ConsensusChange) ConsensusChange {
	return ConsensusChange{
		...
		PiscoinOutputDiffs:        append(cc.PiscoinOutputDiffs, cc2.PiscoinOutputDiffs...),
		FileContractDiffs:         append(cc.FileContractDiffs, cc2.FileContractDiffs...),
		PisfundOutputDiffs:        append(cc.PisfundOutputDiffs, cc2.PisfundOutputDiffs...),
		DelayedPiscoinOutputDiffs: append(cc.DelayedPiscoinOutputDiffs, cc2.DelayedPiscoinOutputDiffs...),
	}
}
