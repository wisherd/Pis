...
import (
	...
	mnemonics2 "github.com/wisherd/Pis/common/entropy-mnemonics"
)
...
func verifyAPISecurity(config Config) error {
	...
	if !config.Pisd.AllowAPIBind {
		addr := modules.NetAddress(config.Pisd.APIaddr)
		if !addr.IsLoopback() {
			if addr.Host() == "" {
				return fmt.Errorf("a blank host will listen on all interfaces, did you mean localhost:%v?\nyou must pass --disable-api-security to bind Pisd to a non-localhost address", addr.Port())
			}
			return errors.New("you must pass --disable-api-security to bind Pisd to a non-localhost address")
		}
		return nil
	}
	...
	if config.Pisd.AllowAPIBind && !config.Pisd.AuthenticateAPI {
...
func processConfig(config Config) (Config, error) {
	var err1, err2 error
	config.Pisd.APIaddr = processNetAddr(config.Pisd.APIaddr)
	config.Pisd.RPCaddr = processNetAddr(config.Pisd.RPCaddr)
	config.Pisd.HostAddr = processNetAddr(config.Pisd.HostAddr)
	config.Pisd.Modules, err1 = processModules(config.Pisd.Modules)
	config.Pisd.Profile, err2 = processProfileFlags(config.Pisd.Profile)
...
func unlockWallet(w modules.Wallet, password string) error {
	...
	dicts := []mnemonics2.DictionaryID{"english", "german", "japanese"}
...
// startDaemon uses the config parameters to initialize Pis modules and start
// siad.
func startDaemon(config Config) (err error) {
	if config.Pisd.AuthenticateAPI {
...
func startDaemonCmd(cmd *cobra.Command, _ []string) {
	...
	profileCPU = strings.Contains(globalConfig.Pisd.Profile, "c")
	profileMem = strings.Contains(globalConfig.Pisd.Profile, "m")
	profileTrace = strings.Contains(globalConfig.Pisd.Profile, "t")
	...
	if profileCPU || profileMem || profileTrace {
		go profile.StartContinuousProfile(globalConfig.Pisd.ProfileDir, profileCPU, profileMem, profileTrace)
	}
